...
測定値は「何かがポーリングしている間」しか増えません。3通りの構成があり、
運用の約束ごとではなく**構造で排他**になっています。

| 構成 | 方法 | トレードオフ |
|---|---|---|
| **デーモン** | `sensor-lens install` | 在席に関係なく収集。API 予算をフルに使う |
| **フロントエンド** | メニューバーアプリが `sensor-lens now --if-stale` を定期実行 | アプリ稼働中だけ収集＝消費が約半分。終了中は欠測 |
| **どちらでもない** | 手動で `now` | 蓄積されない |

混在を安全にしているのが `now --if-stale` です。**最新の測定値が間隔より古い場合だけ**
ポーリングします。デーモンが動いていれば値は新しいので、この tick は **API コール 0**
で終わります。何も動いていなければ、その tick 自身が収集役になります。調整用の
プロトコルも、同期させる設定も要りません。

同一 DB に対するデーモン2重起動は API 消費が黙って倍になるため、`daemon` は排他
ロックを取り、2つ目は起動を拒否します。ロックは `flock` でカーネルが解放するので、
クラッシュしても残留しません。

`status` の `collecting` は **LaunchAgent がロードされているかではなく、最新の測定値の
新しさ**で判定します。launchd しか信じない表示では、メニューバーアプリや手動起動の
デーモンが収集中でも「収集していない」と出てしまうためです。
